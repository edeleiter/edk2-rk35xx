  ...
  )
{
  ...
  /* vcc5v0_host_en (USB_HOST_PWREN) */
  GpioPinWrite (3, GPIO_PIN_PD5, TRUE);
  GpioPinSetDirection (3, GPIO_PIN_PD5, GPIO_PIN_OUTPUT);

  /* vcc5v0_usb30_otg (USB_OTG_PWREN) - Ultra uses GPIO4_PB1, not PB3 */
  ...
  )
{
  ...
  switch (Segment) {
    ...
    case PCIE_SEGMENT_PCIE20L0:
      break;
    case PCIE_SEGMENT_PCIE20L1: // RTL8125B Ethernet
      /* reset */
...
{
  /* Ultra only has HDMI1 (hdmi0 is input/RX mode) */
  if (Id == 1) {
  ...
  )
{
  STATIC CONST EFI_GUID  VendorDtbFileGuid = {
    ...
    0xb399997f, 0x41f6, 0x4441, { 0xaf, 0x14, 0x5c, 0x9a, 0x4b, 0xee, 0xc7, 0x25 }
  };
  STATIC CONST EFI_GUID  MainlineDtbFileGuid = {
    // DeviceTree/Mainline.inf
    0xf4714db4, 0x79a2, 0x46ab, { 0xad, 0x03, 0x43, 0xe6, 0xe6, 0x83, 0x0e, 0xd1 }
  };
